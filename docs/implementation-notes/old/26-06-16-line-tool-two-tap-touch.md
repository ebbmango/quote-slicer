# Line-Tool Two-Tap Touch Interaction
...
Touch devices had no way to trigger line-tool split/merge zones: the split indicators are zero-width and invisible without hover, and there is no hover on touch. This commit introduces a two-tap model — first tap highlights a divisor, second tap on the same one activates — mirroring the UX pattern already used elsewhere for touch-unfriendly precision targets.
...
Each panel's `handleDivisorClick` branches on `isTouch` (from `interactionMedium.svelte.ts`):
...
- **Tool-exit reset**: a `$effect` in QuoteWorkbench clears `touchedDivisor` on leaving line tool, so stale highlights don't persist if the user switches tools.
