...
| `src/lib/breaks.ts`            | Boundary validation and transformations; `tokenMutation.ts` adds explicit identity-safe token edits.                                                                                                                                              |
...
| `src/lib/context/tokenStore.svelte.ts`        | The **token store** — single owner of tokens, canonical tokens and breaks, pinyin overlay, the line-edit animation; `TokenStore` / `TokenAccess` / `EditScope` types ([Token Store](token-store.md))                                                                             |
...
| `src/lib/breaks.spec.ts` / `src/lib/tokenMutation.spec.ts` | server  | Boundary and identity mutation matrix                                                                                    |
...
| `src/lib/context/tokenStore.spec.ts`                       | server  | Token store: canonical tokens and breaks, pinyin overlay (`onMount` is a no-op under node)                               |
