...
| `src/lib/context/breakpoints.svelte.ts`     | `BreakpointContext` — `wide` / `belowMedium` / `tabletPortrait` / derived `minimal` + `dataSurface`, via `matchMedia` ([UI Architecture](ui-architecture.md#responsive-layout))                                                                                                  |
...
| `src/lib/components/ModeToolbar.svelte`           | Bottom toolbar: link/line/view switch + one maps/json toggle pair driven by `BreakpointContext.dataSurface`                                                                                                      |
