...
| `src/lib/context/breakpoints.svelte.ts`     | `BreakpointContext` — reactive wide / narrow / tall `MediaQuery` facts reduced to `layoutMode: 'double' \| 'single' \| 'bottom' \| 'drawer'` ([UI Architecture](ui-architecture.md#responsive-layout))                                                                          |
