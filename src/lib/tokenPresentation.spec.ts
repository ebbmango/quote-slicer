...
describe('tokenPresentation — source (link tool, no font-weight)', () => {
	...
	it('active + focused', () =>
		expect(src(active, true)).toEqual({
			style: `color: ${C}; filter: brightness(0.75);`,
			opacityClass: ''
		}));
	it('idle', () => expect(src(idle, false)).toEqual({ style: '', opacityClass: 'opacity-70' }));
	it('idle + focused', () =>
		expect(src(idle, true)).toEqual({ style: `color: ${C};`, opacityClass: 'opacity-70' }));
	it('unmapped', () =>
		expect(src(unmapped, false)).toEqual({ style: '', opacityClass: 'opacity-30' }));
	it('unmapped + focused', () =>
		expect(src(unmapped, true)).toEqual({ style: '', opacityClass: 'opacity-50' }));
	it('punctuation (null state)', () =>
...
describe('tokenPresentation — target (link tool, font-weight)', () => {
	...
	it('idle', () =>
		expect(tgt(idle, false)).toEqual({ style: 'font-weight: 350;', opacityClass: 'opacity-70' }));
	it('idle + focused', () =>
		expect(tgt(idle, true)).toEqual({
			style: `color: ${C}; font-weight: 350;`,
			opacityClass: 'opacity-70'
		}));
	it('unmapped', () =>
		expect(tgt(unmapped, false)).toEqual({
			style: 'font-weight: 350;',
			opacityClass: 'opacity-30'
		}));
	it('unmapped + focused', () =>
		expect(tgt(unmapped, true)).toEqual({
			style: 'font-weight: 350;',
